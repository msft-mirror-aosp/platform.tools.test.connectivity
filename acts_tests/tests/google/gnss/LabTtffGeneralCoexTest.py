...
class LabTtffGeneralCoexTest(lttb.LabTtffTestBase):
    ...
    def exe_eecoexer_loop_cmd(self, cmd_list):
        for cmd in cmd_list:
    ...
    def gnss_ttff_ffpe_coex_base(self, mode):
        ...
        """
        # Loop all test case in ceoex_testcase_ls
        for test_item in self.ceoex_testcase_ls:

            # get test_log_path from ceoex_testcase_ls['test_name']
            test_log_path = test_item['test_name']

            # get test_cmd from ceoex_testcase_ls['test_cmd']
            test_cmd = test_item['test_cmd']

            # get stop_cmd from ceoex_testcase_ls['stop_cmd']
